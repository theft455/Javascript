...
var wepList = {"glock 18": "D", "p2000": "E", "usp s": "G", "dual berettas": "B", "p250": "F", "tec 9": "H", "five seven": "C", "desert eagle": "A", "galil ar": "Q", "famas": "R", "ak 47": "W", "m4a1 s": "T", "m4a4": "S", "r8 revolver": "J", "ssg 08": "a", "aug": "U", "sg 553": "V", "awp": "Z", "scar 20": "Y", "g3sg1": "X", "nova": "e", "xm1014": "b", "mag 7": "d", "m249": "g", "negev": "f", "mac 10": "K", "mp9": "O", "mp7": "N", "ump 45": "L", "p90": "P", "pp bizon": "M", "zeus x27": "h", "molotov": "l", "incendiary grenade": "n", "decoy grenade": "m", "flashbang": "i", "high explosive grenade": "j", "smoke grenade": "k", "falchion knife": "0", "m9 bayonet": "5", "bayonet": "1", "flip knife": "2", "gut knife": "3", "karambit": "4", "huntsman knife": "6", "bowie knife": "7", "butterfly knife": "8"}
ui_add_sub_tab(["Rage", "SUBTAB_MGR"], "[ dt ]")
var path = ["Rage", "[ dt ]", "[ dt ]"]
var check = ui_add_checkbox(path, "[ dt ] enable")
var style = ui_add_dropdown(path, "[ dt ] style", ["default", "advanced"], 0)
var option = ui_add_dropdown(path, "[ dt ] color theme", ["accent", "doubletap state"], 0)
var accent = ui_add_color_picker(path, "[ dt ] accent")
var xpos = ui_add_slider_float(path, "[ dt ] x pos", 0, parseInt(render_get_screen_size()[0]))
var ypos = ui_add_slider_float(path, "[ dt ] y pos", 0, parseInt(render_get_screen_size()[0]))
...
function draw() {
        ...
        if(ui_get_value(check)==1) {
            ...
            ui_set_enabled(style, 1)
            ui_set_enabled(option, 1)
    
                var alpha = Math.sin(Math.abs(-Math.PI + (global_curtime() * (1 / .25)) % (Math.PI * 2))) * 255
                var cmd = convar_get_int("sv_maxusrcmdprocessticks")
                var speed = ui_get_string(["Rage", "Exploits", "General", "Speed"])
                var local = entity_get_local_player()
                var weapon = entity_get_name(entity_get_weapon(local))
                var current = wepList[weapon]
                var x = ui_get_value(xpos)
                var y = ui_get_value(ypos)
                var textFont = render_get_font("Verdana.ttf", 11, true)
                var weaponFont = render_get_font("undefeated.ttf", 20, false)
                var bulletFont = render_get_font("bullet.ttf", 25, false)
                var color = ui_get_color(accent)
                var text = " [ dt ] | tickbase(v): " + cmd + " | speed: " + speed.toString() + " "
                var resizeText = render_text_size(text, textFont)[0]



                if(ui_get_value(style)==0) {
                    
                    if(ui_get_value(option)==0) {

                    render_filled_rect(x, y, resizeText, 3, color)
                    render_filled_rect(x, y + 3, resizeText, 20, [0, 0, 0, 255])
                    render_string(x, y + 4, 0, text, [255, 255, 255, 255], textFont)
                    ...
                    if(exploit_get_charge()==1) {
                        ...
                        render_filled_rect(x, y, resizeText, 3, [150, 255, 0, 255])
                        render_filled_rect(x, y + 3, resizeText, 20, [0, 0, 0, 255])
                        render_string(x, y + 4, 0, text, [255, 255, 255, 255], textFont)

                } else {

                    render_filled_rect(x, y, resizeText, 3, [255, 0, 50, 255])
                    render_filled_rect(x, y + 3, resizeText, 20, [0, 0, 0, 255])
                    render_string(x, y + 4, 0, text, [255, 255, 255, 255], textFont)

                    }
                } if(wepList.hasOwnProperty(weapon)) {
                        render_string(x, y + 25, 0, current, [255, 255, 255, 255], weaponFont)
                    }
                }



                if(ui_get_value(style)==1) {

                    ui_set_enabled(option, 0)

                    render_filled_rect(x, y, resizeText, 20, color)
                    render_string(x, y + 2, 0, text, [0, 0, 0, 255], textFont)

                    if(exploit_get_charge()==1) {

                        render_filled_rect(x - 4, y, 4, 20, [150, 255, 0, 255])
                        render_string(x + 70, y + 20, 0, "AA", [255, 255, 255, 255], bulletFont)

                    } else {

                        render_filled_rect(x - 4, y, 4, 20, [255, 0, 50, 255])
                        render_string(x + 70, y + 20, 0, "AA", [255, 255, 255, alpha], bulletFont)     

                    } if(wepList.hasOwnProperty(weapon)) {
                        render_string(x - 4, y + 25, 0, current, [255, 255, 255, 255], weaponFont)
            }
        }
    }
}
